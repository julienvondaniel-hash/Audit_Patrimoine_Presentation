...
## Application web — Generateur d'etude patrimoniale

Le dossier [`docs/`](docs/) contient une **application 100 % navigateur** (aucun
serveur) qui produit une presentation PowerPoint (`.pptx`) d'**etude patrimoniale**
a partir de donnees saisies dans un formulaire (profil du foyer, actifs, budget,
audit successoral, objectifs, preconisations, plan d'action) et de modules
pedagogiques activables (PER, assurance-vie, PEA, PEA-PME, FCPR, SCPI, SCI a l'IS).

- Utilisation et personnalisation : voir [`docs/README.md`](docs/README.md).
- Mise en ligne : GitHub Pages (Settings → Pages → Source « GitHub Actions »,
  workflow [`.github/workflows/deploy-pages.yml`](.github/workflows/deploy-pages.yml)),
  ou « Deploy from a branch » sur le dossier `/docs`.
- En local : ouvrir directement `docs/index.html` dans un navigateur.

Cette application est independante du tableau de bord mensuel decrit ci-dessus.
